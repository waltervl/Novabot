...
export function LiveStatusCard({ sn }: Props) {
  ...
  return (
    <div className="bg-zinc-900 rounded-lg border border-zinc-800 p-3 mb-3">
      <h3 className="text-xs font-semibold text-zinc-300 mb-2">Live status</h3>

      {error && <p className="text-xs text-red-400">Failed: {error}</p>}
      {!error && missing && <p className="text-xs text-zinc-500">No LoRa cache for this mower yet.</p>}
      {!error && !missing && data && (
        <div className="space-y-1.5 text-xs">
          <div className="flex items-center justify-between">
            <span className="inline-flex items-center gap-1.5 text-zinc-400">
              <Radio className="w-3 h-3" /> Pair
            </span>
          ...
          {data.peer.sn && (
            <div className="flex items-center justify-between text-[11px]">
              <span className="text-zinc-500 font-mono pl-4">peer: {data.peer.sn}</span>
          ...
          <div className={`flex items-center justify-between pt-1 border-t border-zinc-800 ${data.drift ? 'text-amber-400' : 'text-emerald-400'}`}>
            <span className="inline-flex items-center gap-1.5">
              ...
              {data.drift ? 'Drift' : 'In sync'}
            </span>
            <span>{data.drift ? 'mismatch' : 'matching'}</span>
          </div>
        </div>
      )}
      {!error && !missing && !data && <p className="text-xs text-zinc-500">Loading…</p>}
    </div>
  );
}
