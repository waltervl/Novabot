...
Used by test_field_name_verification.py to cross-check source code
references against the live interface definitions cached at
`research/ros2_msg_definitions/`.

Format we accept (subset of the rosidl IDL):
- Lines starting with '#' are comments. Ignore.
- Blank lines: ignore.
- '---' separators in .srv split request/response; in .action split
  goal/result/feedback.
- Constant declaration: `<type> <NAME>=<value>` — record but don't expose
  as fields.
- Field declaration: `<type> <name>` (with optional `<type>[]` array
  marker, optional default value separated by whitespace).
"""
