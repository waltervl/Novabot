"""Walk mower/*.py via ast and pull every (TypeName, field, section)
triple where the source assigns to a field on a Request/Goal/message
or reads a field off a Response/Result.

Returns a list of `Reference(file, line, type_name, field, kind)` so
the test can locate any miss precisely.
"""
