...
void walkerLoraPump() {
    ...
    while (loraSerial.available()) {
        ...
        switch (g_st) {
            ...
            case LP_POST2:
                if (b == 0x03) {
                    // Valid LoRa frame. The charger emits a mixed UM960 stream
                    // on 0x31: RTCM3 corrections plus NMEA / proprietary bytes.
                    // Forward the payload verbatim, matching the proven mower
                    // path; the LC29 extracts RTCM internally. PAIR commands are
                    // protected separately by temporarily pausing forwarding.
                    g_framesReceived++;
